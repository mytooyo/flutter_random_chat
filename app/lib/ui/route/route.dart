...
class Router {
  ...
  static Route<dynamic> onGenerateRoute(RouteSettings settings) {
    ...
    if (self != null && !self.available && ![RouteName.start, RouteName.home].contains(settings.name)) {
    ...
    switch (settings.name) {
      ...
      case RouteName.start:
        return StandardPageRoute(
          builder: (_) => UserProfileScreen(user: null, editable: true, initial: true,),
        );
      ...
      case RouteName.messageDetail:
        final MessageDetailScreenArgument args = settings.arguments;
        return FadeAnimationRoute<void>(
          builder: (_) => MessageDetailScreen(message: args.message, type: args.type, callback: args.callback,),
          fullscreenDialog: true,
        );
      
      case RouteName.showImage:
        final MessageImageScreenArgument args = settings.arguments;
        return PageRouteBuilder(
          opaque: false,
          pageBuilder: (BuildContext context, _, __) => MessageImageScreen(img: args.img, file: args.file,),
          fullscreenDialog: true,
        );

      case RouteName.userProfile:
        final UserProfileScreenArgumanet args = settings.arguments;
        return FadeAnimationRoute<void>(
          builder: (_) => UserProfileScreen(user: args.user, editable: args.editable,),
          fullscreenDialog: true,
        );
      ...
      case RouteName.conversation:
        final ConversationScreenArgument args = settings.arguments;
        return StandardPageRoute(
          builder: (_) => MultiProvider(
            providers: [
            ...
            child: ConversationScreen(message: args.message, user: args.user, conv: args.conv, callback: args.callback,),
          )
        );

      case RouteName.post:
        return StandardPageRoute(
          builder: (_) => PostMessageScreen(),
          fullscreenDialog: true,
        );
